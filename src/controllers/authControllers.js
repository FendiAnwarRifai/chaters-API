...
const auth = {
    register: (req, res) => {
        ...
        model.users.findAll({ where: { email: data.email } })
            .then((result) => {
                if (result.length > 0) {
                    return helper.response('warning', res, null, 200, 'email is already in use')
                }
                data.id = uuidv4()
    ...
    login: (req, res) => {
        ...
        model.users.findAll({ where: { email: email } })
            .then((result) => {
                let user = result[0]
            ...
            .catch((err) => {
                helper.response('error', res, err, 401, 'email is not specified')
            })
    }
}
module.exports = auth